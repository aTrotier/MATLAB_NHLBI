...
% 05/07/2013 vds_GRT.m
% Adrienne Campbell-Washburn modified to produce gradient
% waveforms at the Siemens gradient raster time (GRT)
...
while r0 < rmax
	[q2,r2] = findq2r2(smax,gmax,r0,r1,T,Ts,N,Fcoeff,rmax);
...
r2 = r(2:count);
theta2 = theta(2:count);
time2 = time(2:count)-time(2);
clear r theta time; 
r = r2; theta = theta2; time = time2; 

k = r.*exp(i*theta);

g = 1/gamma*([k 0]-[0 k])/Ts/oversamp;
g = g(1:length(k));

s = ([g 0]-[0 g])/Ts/oversamp;
s = s(1:length(k));
...
function [q2,r2] = findq2r2(smax,gmax,r,r1,T,To,N,Fcoeff,rmax)
...
GmaxFOV = 1/gamma /F/T;		% FOV limit on G
...
%		tt = sprintf('Slew violation, slew = %d, smax = %d, sr=%f, r=%f,
%		r1=%f',round(abs(slew)),round(smax),sr,r,r1);%
		%disp(tt);
%	end;
